...
function OrderSummary({ items, total, submitting }: { items: CartItem[]; total: number; submitting: boolean }) {
  return <aside className="order-summary checkout-panel">
    ...
    <div className="summary-total"><span>المجموع الفرعي</span><b>{total.toFixed(2)} ر.س</b><span>التوصيل</span><b className="checkout-free">مجاني</b><strong>الإجمالي</strong><strong>{total.toFixed(2)} ر.س</strong></div>
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/orders", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
        firstName: form.get("firstName"), lastName: form.get("lastName"), phone: form.get("phone"), city: form.get("city"), address: form.get("address"), notes: form.get("notes"), paymentMethod: "cod",
